// © Team King OFC
// Bot By OFC The King GT-502 Modz
// Suscríbete
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:The King OFC\n' 
            + 'ORG:Creador De BOT\n' 
            + 'TEL;type=CELL;type=VOICE;waid=526567880086:+52 656 788 0086\n' 
            + 'END:VCARD'
//WhatsApp Conexión
const
	{
...
async function starts() {
	...
	console.log(color('[ BOT]','aqua'), color("BOT by OFC The King GT-502 Modz", "yellow"))
	console.log('>', '[',color('INFO','blue'),']','Suscribete A OFC The King GT-502 Modz...')
	FxBot.on('qr', () => {
	...
	FxBot.on('open', () => {
	console.log(color('> [ INFO ]', 'white'), color('Ya Estoy Conectado 🖤🥀'))
	})
...
┏━━━━━━━━━━━━━━━━━
┠⊷️ *Suscríbete* :
┠⊷️*https://youtube.com/c/OFCTheKingGT502*
┠⊷️ *Grupo OFC* :
┠⊷️https://chat.whatsapp.com/IXnlAaTfM5iByQiNW7d3Yh
┗━━━━━━━━━━━━━━━━━
┏━━━━━━━━━━━━━━━━━
┠⊷️「 *DESCRIPCIÓN* 」
┗━━━━━━━━━━━━━━━━━
       ...
       const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
		...
		mess = {
			...
			only: {
				...
				admin: '「 ❗ 」No Eres Admin Pvta De The King GT',
...
//========================================================================================================================//
		...
		if (isCmd && isBanned) {
        return console.log(color('[ USUÁRIO BANEADO ] Ignorando comando', 'blue'), color(moment.tz('America/Guatemala').format('HH:mm:ss'), 'yellow'), color(`${command}`),'DE:', color(pushname))}
...
//HIDETAG
case prefix+ 'hidetag':
			if (!mek.key.fromMe) return fakestatus('THEKING-BOT')
    ...
    case prefix+ 'sticktag':
			...
			case prefix+ 'doctag':
			  //by Dehanjing
			    ...
			    break
    case prefix+ 'contag':
            if (!mek.key.fromMe) return reply('THEKING-BOT')
            pe = args.join('')
            entah = pe.split('|')[0]
            nah = pe.split('|')[1]
...
if (!isGroup) return reply(mess.only.group)
					...
					mentions(`Adios Chupa Pijas @${mentioned[0].split('@')[0]} 🥵`, mentioned, true)
					FxBot.groupRemove(from, mentioned)
					}					
					break 
					case prefix+ 'promote':
...
case prefix+ 'leave':
					...
					FxBot.groupLeave (from) 
                     ...
                     setTimeout( () => {
					FxBot.updatePresence(from, Presence.composing) 
					FxBot.sendMessage(from, 'Adios Pvtas De The King GT', text) // ur cods
					}, 0)
                     break
...
case prefix+ 'tagall2':
				...
				FxBot.updatePresence(from, Presence.composing) 
					...
					for (let mem of groupMembers) {
						teks += ` ꪶ͢♞ꫂ⛧ ${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					FxBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'✪ 𝚃𝙺┃𝙱𝙾𝚃 ✪', text, {quoted: mek})
					break
                case prefix+ 'tagall3':
                case prefix+ 'marcar3':
				FxBot.updatePresence(from, Presence.composing) 
...
case prefix+ 'tovid':
    ...
    case prefix+ 'toimg':
			...
			fakethumb(buffer,' ꪶ͢♞ꫂ⛧꙳𝔱ⷪ𝔥ᷫ𝔢ᷬ𝔨ⷭ𝔦ᷬ𝔫ᷧ𝔤ᷞ꙳ཏ꯰2⛧ꪶ͢♞')
			fs.unlinkSync(ran)
			})
			break
case prefix+ 'tourl':
...
.then((a) => {
if (Number(filesize) >= 30000) return sendMediaURL(from, thumb, `*DESCARGADOR DE AUDIO BY The King GT*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*:MP3\n*⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración de más del límite se presenta en forma de enlace_`)
const captions = `*DESCARGADOR DE AUDIO BY  The King GT*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*:MP3\n*⚖️Tamaño*:${filesizeF}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES ALV_`
...
reply(mess.mpv)
				...
				axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
				.then((a) => {
				if (Number(filesize) >= 40000) return sendMediaURL(from, thumb, `*DESCARGADOR DE VIDEO BY  The King GT*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*:MP3\n*File⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración de más del límite se presenta en forma de enlace_`)
	...
	case prefix+ 'ytsearch':
			if (args.length < 1) return reply('Que quieres buscar traga nepes?')
			var srch = args.join('');
			try {
...
case prefix+ 'play':
   			...
   			var mulaikah = aramat[0].url							
                  try {
                    ...
                    .then((res) => {
                        ...
                        axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                        .then(async (a) => {
                        if (Number(filesize) >= 100000) return sendMediaURL(from, thumb, `*DESCARGADOR DE MUSICA BY  The King GT*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*: MP3\n*⚖️Tamaño*: ${filesizeF}\n*📎Link* : ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES PVTA MIA_`)
		                        const captions = `*DESCARGADOR DE MUSICA BY  The King GT*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*: MP3\n*⚖️Tamaño*: ${filesizeF}\n*📎Link*: ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES CAREPIJA_`
                        sendMediaURL(from, thumb, captions)
                  ...
                  try {
                    ...
                    .then((res) => {
                        ...
                        if (Number(filesize) >= 100000) return sendMediaURL(from, thumb, `*PLAY VIDEO*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*:MP3\n*File⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES CAREPIJA_`)
		                        const captions = `*PLAY VIDEO*\n\n*🔥Titulo*: ${title}\n*⏭️Extensión*:MP4\n*⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES PVTA_`
                        sendMediaURL(from, thumb, captions)
                        await sendMediaURL(from, dl_link).catch(() => reply('error'))
                        })                
                        })
    ...
    FxBot.sendMessage(from, options, text, { quoted: mek } )
				break
case prefix+ 'creador':
await FxBot.sendMessage(from, {displayname: " TheKingGT", vcard: vcard}, MessageType.contact)
break
